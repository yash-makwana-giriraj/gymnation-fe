...
import { ContentItem, DynamicComponentProps, GlobalComponentProps } from '@/interfaces/content';

// Lazy-load components using Next.js dynamic imports
const AboutUsHeroSlider = dynamic(() => import('@/components/banners/AboutUsHeroSlider'), { ssr: true });
const LocationCard = dynamic(() => import('@/components/sliders/LocationCard'), { ssr: true });
...
const componentMap: Record<string, ComponentType<DynamicComponentProps>> = Object.fromEntries(
    Object.entries(components).map(([key, component]) => [
        key.toLowerCase().replace(/([A-Z])/g, (_match, char: string) => char.toLowerCase()),
        component,
    ])
);
...
const GlobalComponentsRendor = memo(function GlobalComponentsRendor({ apiData }: GlobalComponentProps) {
    ...
    return (
        <div>
            {contentItems.map((item, index) => {
                ...
                const Component = componentMap[normalizedContentType] || FallbackComponent;
