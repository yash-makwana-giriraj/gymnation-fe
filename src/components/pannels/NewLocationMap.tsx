import { APILocationsResponse, ContentResponse, DynamicComponentData, FeaturesItem, Item, Properties } from '@/interfaces/content'
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import MapBoxMap, { MapBoxRef } from '../map/MapBoxMap';
import { fetchCityLocationFilters, fetchCityWithLocationData } from '@/api-handler/apis/content';
...
const NewLocationMap = ({ data }: { data: DynamicComponentData }) => {
    ...
    // ** New function to get filtered locations based on appliedFilters **
    ...
    return (
        <section className="bg-primary global-spacing map-container !pt-32">
            <div className="container mx-auto">
                <div className="grid grid-cols-1 sm:grid-cols-[420px_minmax(200px,1fr)] xslg:grid-cols-[minmax(605px,1fr)_minmax(200px,1fr)] lg:grid-cols-[430px_1fr] text-white overflow-hidden w-full">
                    <div className="block !pl-0 !pt-0 px-[15px] pb-[16px] mb:px-[12px] xs:py-[25px] xs:px-[35px] sm:p-[20px] slg:pl-[25px] bg-primary sm:min-h-[530px]">
                  ...
                  <h2 className="text-red text-center text-[16px] font-600 leading-[28px] mb-[22px] uppercase">
                    No Location Found
                            ...
                            <Swiper
                                ...
                                className="location-card-swiper max-h-[640px]"
                            >
                        ...
                        <div className="hidden xs:block sm:hidden w-[calc(100%_+_35px)]">
                            ...
                            >
                                {filteredLocations.map((location: APILocationsResponse, index: number) => (
                                    <SwiperSlide
                                        key={index}
                                        className="group swiper-slide-active:!group !h-auto !min-h-[192px]"
                                    >
                                        <LocationDetailCard
                                            index={index}
                                            title={location.name}
                    ...
                    <div className="relative w-full h-[530px] mb:h-[703px] xs:h-[280px] sm:h-[830px] xs:bg-primary xs:px-[35px] xs:pb-[45px] sm:p-[0]">
